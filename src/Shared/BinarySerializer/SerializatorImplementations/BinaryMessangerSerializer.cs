...
public class BinaryMessangerSerializer : BinaryBase, IDisposable
{
    ...
    public BinaryMessangerSerializer(Stream baseStream, int initialStreamCapacity, BinaryWriter binaryWriter, bool disposed) : base(baseStream, initialStreamCapacity)
    {
        _binaryWriter = binaryWriter;
        _disposed = disposed;
    }
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public BinaryMessangerSerializer Write(List<Guid> guids) 
    {
        if (guids.Count == 0)
        {
            _binaryWriter.Write(EmptyArrayHeader);
        ...
        _binaryWriter.Write(guids.Count);
